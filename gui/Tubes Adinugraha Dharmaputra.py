...
def title():
    waktu = strftime('%H:%M')
    tanggal = str(cal.selection_get())
    root.title(tanggal + " | " + waktu + "| Calendar Adi")
...
root.title("Calendar Adi")


cal = Calendar(root, font = 'Calibri 14', selectmode='day', locate='id_ID', cursor='hand2', background='dodgerblue', weekendbackground='red')
cal.grid(row=0, column=0,  sticky='N', rowspan=7, columnspan=3)
cal.bind("<<CalendarSelected>>",ListTodo)
tanggal = str(cal.selection_get())
treev = ttk.Treeview(root)
treev.grid(row=9, column=0, sticky='WNE', rowspan=4, columnspan=2)
scrollBar = tk.Scrollbar(root, orient='vertical', command=treev.yview)
scrollBar.grid(row=9, column=2, sticky="ENS", rowspan=4)
treev.configure(yscrollcommand=scrollBar.set)
treev.bind("<Double-1>", detailTodo)
treev['columns'] = ('1','2')
treev['show'] = 'headings'
treev.column("1", width=120)
treev.heading("1", text="Jam")
treev.heading('2', text="Judul")

btnAdd = tk.Button(root, text="Tambah", bg='dodgerblue', fg="white", width=22, cursor='hand2', command=AddForm)
btnAdd.grid(row=7, column=1, sticky='N')

btnDel = tk.Button(root, text="Hapus", bg='dodgerblue', fg="white", width=22, cursor='hand2', command=delTodo)
btnDel.grid(row=7, column=0, sticky='N')

btnSave = tk.Button(root, text="Save", bg='dodgerblue', fg="white", width=22, cursor='hand2', command=SaveTodos)
btnSave.grid(row=8, column=1, sticky='S')

btnExit = tk.Button(root, text="Exit", bg='blue', fg="white", width=22, cursor='hand2', command=root.destroy)
btnExit.grid(row=8, column=0, sticky='S')

tk.Label(root,text="Calendar Adinugraha Dharmaputra", bg='dodgerblue', fg='white').grid(row=13, column=0, columnspan=3)
